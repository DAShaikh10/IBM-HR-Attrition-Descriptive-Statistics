...
```{r job-sat-vs-attrition, fig.height = 3, fig.width = 5, fig.pos = "H", fig.cap = "Attrition counts across Job Satisfaction levels. Employees reporting 'Low' satisfaction show a higher relative frequency of attrition compared to those in the 'High' and 'Very High' categories"}
...
knitr::kable(tab_js_attr_margins, caption = "Attrition by Job Satisfaction Level (with Marginal Frequencies)", align = c('l', 'c', 'c', 'c', 'c', 'c'))
...
knitr::kable(round(tab_js_rowprop, 2), caption = "Row Relative Frequency: Job Satisfaction within Attrition Groups", align = c('l', 'c', 'c', 'c', 'c'))
...
knitr::kable(round(income_by_attrition, 2), caption = "Monthly Income Summary by Attrition (Mean, Median, SD)")
...
cor_table <- data.frame(
  Statistic = c("Pearson correlation", "Spearman correlation"),
  Value = round(c(
    cor(hr$MonthlyIncome, attrition_numeric, method = "pearson"),
    cor(hr$MonthlyIncome, attrition_numeric, method = "spearman")
  ), 3)
)
knitr::kable(cor_table, caption = "Correlation Coefficients: Monthly Income vs Attrition", align = c('l', 'c'))
```
...
```{r demographics-attrition, fig.cap="Demographic Drivers of Attrition: (Left) Age distribution showing a lower median age for leavers; (Right) Tenure distribution showing early-stage attrition among new hires."}
...
# 1. Age vs Attrition
boxplot(Age ~ Attrition, data = hr,
        main = "Age by Attrition",
        xlab = "Attrition", ylab = "Age",
        col = c("lightblue", "salmon"))

# 2. Years at Company vs Attrition
boxplot(YearsAtCompany ~ Attrition, data = hr,
        main = "Tenure by Attrition",
        xlab = "Attrition", ylab = "Years at Company",
...
par(mar = c(5, 4, 4, 2) + 0.1)
```
**Interpretation:** he analysis confirms that attrition is primarily an early-career phenomenon. Employees who leave are generally younger (median age 32 vs. 36) and have significantly shorter tenure (median 3 years vs. 6 years) compared to those who stay. This indicates that the organization struggles most with retaining new and junior talent, rather than long-term employees.
...
plot(hr$YearsAtCompany, hr$MonthlyIncome,
...
legend("bottomright", legend = c("Data", "Regression Line"),
  col = c("#377eb8", "#e41a1c"),
  pch = c(19, NA), lty = c(NA, 1), lwd = c(NA, 2),
  bty = "n", pt.cex = 0.8, cex = 0.85, x.intersp = 1.1, y.intersp = 1)

# Regression summary as a table (no duplicate row index)
reg_summary <- summary(lm_fit)
reg_table <- data.frame(
  Term = c("Intercept", "Years at Company"),
  Estimate = round(reg_summary$coefficients[,1], 2),
  `Std. Error` = round(reg_summary$coefficients[,2], 2),
  `t value` = round(reg_summary$coefficients[,3], 2),
  `Pr(>|t|)` = format.pval(reg_summary$coefficients[,4], digits = 2, eps = .001)
)

knitr::kable(reg_table, caption = "Regression Coefficients: Monthly Income ~ Years at Company", align = 'c', row.names = FALSE)

# Model metrics as a horizontal table
metrics_vec <- c(
  round(reg_summary$r.squared, 3),
  round(reg_summary$adj.r.squared, 3),
  round(reg_summary$fstatistic[1], 2),
  format.pval(pf(reg_summary$fstatistic[1], reg_summary$fstatistic[2], reg_summary$fstatistic[3], lower.tail = FALSE), digits = 2, eps = .001)
)
metrics_table_horiz <- data.frame(
  R_squared = metrics_vec[1],
  Adjusted_R_squared = metrics_vec[2],
  F_statistic = metrics_vec[3],
  p_value = metrics_vec[4]
)
knitr::kable(metrics_table_horiz, caption = "Model Fit Statistics (Horizontal)", align = 'c', row.names = FALSE)
```
