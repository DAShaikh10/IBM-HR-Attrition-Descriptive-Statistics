...
header-includes:
  ...
  - \pagestyle{fancy}
  - \fancyhf{}
  - \fancyhead[R]{\includegraphics[height=1cm]{assets/bht-logo.pdf}}
...
print_freq_table <- function(x,
                             ...
                             show_mode = FALSE) {
  # Create frequency table and rename columns
  ...
  # Optionally add cumulative frequencies
  if (show_cumulative) {
    df$`Cum. Freq.` <- cumsum(df$Frequency)
    df$`Cum. %`     <- cumsum(df$Proportion)
  }
  
  # Optionally highlight modal category
  if (show_mode) {
...
print_mode <- function(x) {
  # Get unique values and find most frequent one using tabulate and match
  uniqv <- unique(x)
...
analyze_numeric <- function(data_vec,
                            ...
                            col_border,
                            col_line) {
  # Calculate quartiles and create statistics data frame
  ...
  # Print statistics table (needed for rendering inside function)
  print(knitr::kable(
    stats_df,
    align   = 'c',
    caption = paste(
  ...
  # Create side-by-side boxplot and ECDF
  par(mfrow = c(1, 2))
  par(mar = c(4, 4, 2, 1)) # Reduce margins.
  
  # Boxplot showing dispersion and outliers
  boxplot(
    data_vec,
  ...
  # ECDF plot showing cumulative distribution
  plot(
    ecdf(data_vec),
...
# Barplot using ggplot [@R-ggplot2]
ggplot(hr,
...
```{r job-satisfaction-univariate, fig.width = 3.5, fig.height = 2.5, fig.cap = "Distribution of Job Satisfaction Levels", fig.pos = "H"}
...
# Barplot using ggplot [@R-ggplot2]
ggplot(hr, aes(x = JobSatisfaction, fill = JobSatisfaction)) +
  geom_bar(width = 0.7) +
  geom_text(
    stat = "count",
    aes(label = ..count..),
  ...
  labs(
    title = "Distribution of Job Satisfaction Levels",
    x = "Job Satisfaction",
...
**Interpretation:** The data indicates **high** employee morale, with **"Very High"** _(31.22%, n=459)_ as the **modal** category. Cumulatively, over **61.29%** of the workforce reports **positive satisfaction** _(High or Very High)_, significantly outweighing the **19.66%** who report **low** satisfaction.
...
**Interpretation:** The workforce structure exhibits a distinct pyramidal hierarchy, heavily concentrated in early-career roles. **Entry Level** _(36.94%, n=543)_ and **Junior** _(36.33%, n=534)_ positions **dominate**, cumulatively accounting for **73.27%** of the organization. In contrast, upper management is exclusive, with **Executives** representing only **4.69%** of the total population.
...
# Plot histogram with overlaid mean and median lines
hist(
  hr$MonthlyIncome,
...
abline(v = avg_inc, col = "firebrick", lwd = 2, lty = 2)
abline(v = med_inc, col = "navy", lwd = 2, lty = 1)
...
**Interpretation:** The distribution of tenure is **strongly right-skewed**, evidenced by the **Mean** _(7.01)_ exceeding the **Median** _(5.0)_. The boxplot reveals significant **upper outliers** _(up to 40 years)_, indicating that while the typical employee has a tenure of 5 years or less, a distinct cohort of long-term veterans remains, heavily influencing the average. The ECDF confirms this early-career concentration with a sharp vertical rise between 0 and 10 years, showing that approximately **75%** of the workforce has been with the company for **9 years or less** _(3rd Quartile)_, before the curve flattens out for the remaining long-term employees.
...
# Grouped barplot comparing attrition across satisfaction levels
barplot(
  tab_js_attr,
  beside = TRUE,
  main   = "Attrition by Job Satisfaction",
  legend = rownames(tab_js_attr),
  args.legend = list(x = "topright", bty = "n", cex = 0.8, inset = c(-0.03, -0.05)),
...
**Interpretation:** The analysis shows a clear association between job satisfaction and attrition. Among employees who left the organization, **28%** reported Low satisfaction, compared to 18% among those who stayed, while Very High satisfaction is more common among retained employees. The bar chart visually confirms this trend: as **satisfaction levels rise**, the volume of **retained employees** _(Blue)_ **increases** substantially,although attrition does not disappear entirely at higher satisfaction levels, indicating that job satisfaction alone does not fully explain attrition.
...
**Interpretation:** Employees who left the organization earn **lower monthly incomes** than those who stayed. The mean monthly income for employees who stayed is _USD 6,832.74 (median: USD 5,204)_, compared to _USD 4,787.09 (median: USD 3,202)_ for employees who left. The boxplot shows that income among employees who left is concentrated at **lower levels** with **reduced variability**, while employees who stayed exhibit **higher incomes** and **greater dispersion**. This indicates an association between lower compensation and higher attrition, though the findings are descriptive and do not imply causality.

\newpage

## Demographics & Attrition (Discrete / Continuous vs. Nominal)
...
# Side-by-side boxplots comparing age and tenure by attrition status
boxplot(Age ~ Attrition, data = hr, main = "Age by Attrition",
        xlab = "Attrition", ylab = "Age", col = c("lightblue", "salmon"))

boxplot(YearsAtCompany ~ Attrition, data = hr, main = "Tenure by Attrition",
        xlab = "Attrition", ylab = "Years at Company", col = c("lightgreen", "salmon"))
...
## Correlation Analysis: Years at Company vs. Monthly Income (Discrete vs. Continuous)
...
# Density scatter plot with regression line
smoothScatter(
  hr$YearsAtCompany, hr$MonthlyIncome,
  xlab = "Years at Company", ylab = "Monthly Income (USD)",
  main = "Monthly Income vs Years at Company",
  colramp = colorRampPalette(c("white", "#377eb8")), pch = 19
)

abline(lm_fit, col = "#e41a1c", lwd = 2)
legend("bottomright", legend = c("Data Density", "Regression Line"),
       fill = c("#377eb8", "#e41a1c"), bty = "n", cex = 0.85)

par(mar = c(5, 4, 4, 2) + 0.1)

# Extract regression parameters for equation
...
knitr::kable(coeffs, align = 'c', caption = "Linear Regression Coefficients")

# Model fit statistics
fit_stats <- data.frame(
  Measure = c("R-Squared", "Residual Std. Error (RSE)", "Observations"),
  Value   = c(round(lm_sum$r.squared, 4), round(lm_sum$sigma, 2),
              format(nobs(lm_fit), big.mark = ","))
)

knitr::kable(fit_stats, align = 'l', caption = "Model Goodness of Fit")
```
