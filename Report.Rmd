...
pearson_r  <- round(cor(hr$MonthlyIncome, attrition_numeric, method = "pearson"), 2)
spearman_r <- round(cor(hr$MonthlyIncome, attrition_numeric, method = "spearman"), 2)
```

**Interpretation:** Employees who left the organization earn lower monthly incomes than those who stayed, as reflected in both mean and median values.The weak negative Pearson (**`r pearson_r`**) and Spearman (**`r spearman_r`**) correlations indicate that higher income is associated with a lower likelihood of attrition.
...
Following the bivariate analysis, which identified a moderate positive correlation ($r = 0.51$) between tenure and compensation, this section utilizes Simple Linear Regression to quantify this relationship. The goal is to model Monthly Income (Dependent Variable) as a function of Years at Company (Independent Variable). By estimating the regression coefficients, we aim to determine the financial premium associated with employee longevity and assess how well tenure predicts overall earnings.
