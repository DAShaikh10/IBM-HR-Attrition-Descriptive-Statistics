...
geometry: "left=0.4in, right=0.4in, top=0.8in, bottom=0.8in"
fontsize: 12pt
---
...
```
The distribution of runs_total shows that most deliveries result in 0 or 1 run, while boundaries (4 or 6 runs) occur less frequently but have a large impact on total scoring.

## 3.2 Batter
The variable batter records the striker on each delivery. The absolute frequency counts describe number of balls faced by the batter.
```{r batter}
table(match$batter)
```
...
```{r}
quantile(match$runs_total, probs = c(0, 0.25, 0.5, 0.75, 1))
```
